function Output = CreateOutput(Archive)


for j=1:numel(Archive)
    Archive(j).nps=Archive(j).cost;
end

for i=numel(Archive):-1:2
    for j = 1:i-1
        if isequal(Archive(i).nps,Archive(j).nps)
            Archive(i).nps=[];
            break
        end
    end
