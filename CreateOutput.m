...
for j=1:numel(Output.F1)
    Output.F1(j).nps=Output.F1(j).Cost;
end

for i=numel(Output.F1):-1:2
    for j = 1:i-1
        if isequal(Output.F1(i).nps,Output.F1(j).nps)
            Output.F1(i).nps=[];
            break
        end
    end
...
h=0;
for i=1:numel(Output.F1)
    if ~isempty(Output.F1(i).nps)
        h=h+1;
    end
end
