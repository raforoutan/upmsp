function [Popnew,Updatelist]=CandidateSolutions(pop,Paretolist,Candidatelist)
        ...
        N=zeros(1,t);        % dominate Counter
         ...
         for i=1:J
            for j=i+1:t              
               if all(Archive(i).Z<=Archive(j).Z)&& any(Archive(i).Z<Archive(j).Z) % if i dominated j
                  N(j)=N(j)+1;
               elseif all(Archive(j).Z<=Archive(i).Z)&& any(Archive(j).Z<Archive(i).Z)  % if j dominated i
                  N(i)=N(i)+1;
               end
            end
           if N(i)==0
